...
[CreateAssetMenu]
    public class TVCraftingRecipe : ScriptableObject
    {
                ...
                foreach(ItemAmount itemAmount in results)
                {
                    for (int i = 0; i < itemAmount.amount; i++)
                    {
                        ...
                        if(buildAmount == 0)
                        {
                            ...
                            GameObject aux = Instantiate(itemAmount.trapPrefab, GameVars.Values.GetPlayerPrefabPlacement(), Quaternion.Euler(-90f, 0f, -180f));
